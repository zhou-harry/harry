sap.ui.define([ 
	...
	'harry/app/format/formatter'
				...
				formatter) {
	"use strict";
	return Controller.extend("harry.app.controller.role.RoleManager", {
...
//	}，
	...
	onRoleAdd: function(oEvent){
		...
		if (!oDialog) {
			...
			oDialog.setModel(new JSONModel({isSys:false,status:true}), "role");
...
//				oModel.refresh();
	...
	onRoleDelete: function(oEvent){
		...
		oView.byId("role").setBusy(true);
		this.JsonModel().loadData(this,
			"role/remove?roleid="+_data[index].roleId,
			function (oData, model) {
	...
	onRoleSelected: function(oEvent){
		...
		oTree.setBusy(true);
		this.JsonModel().loadData(this,
			"menu/treeSide?roleId="+cData.roleId+"&parentKey=SIDE",
			function (oData, model) {
				oTree.destroyItems();
	...
	getChildNode : function(item) {
		...
		var oTree = item.getTree(),
			iNodeLevel = item.getLevel(),
			i = oTree.indexOfItem(item) + 1,
