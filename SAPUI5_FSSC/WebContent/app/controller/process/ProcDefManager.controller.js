sap.ui.define([ 
	...
	"use strict";
	return BaseController.extend("harry.app.controller.process.ProcDefManager", {
		formatter: formatter,
	/**
		...
		this.JsonModel().loadData(this,
				...
				function (oData, model) {
					...
					$.each(oData.data, function(index, ele) {
						...
						var date = new Date(sdate[0], sdate[1] - 1, sdate[2], stime[0],
								stime[1], stime[2]);
						ele.deployTime = date;
						this.JsonModel().loadData(this,
								"bpm/sizeInstance?pdid="+ele.id,
								function (oData, model) {
	...
	onPressDiagram: function (oEvent) {
		...
		this.getView().setModel(new JSONModel({diagram:url}));
		if (!this.oDiagramPopover) {
	...
	confirmDeploy : function () {
		...
		this.JsonModel().loadData(this,
				"bpm/deploy?name="+sText+"&type="+sType,
				function (oData, model) {
					this._initTypeTree();
	...
	handleDelete: function(oEvent) {
		...
		context = oItem.getBindingContext("detail"),
		...
		this.JsonModel().loadData(this,
				"bpm/deleteDeploy?deployId="+deploymentId,
				function (oData, model) {
					if (oData.statusCode == "000000") {
						this._initTypeTree();
