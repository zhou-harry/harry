sap.ui.define([
		...
		"use strict";
		return BaseController.extend("harry.app.controller.process.ProcessList", {
			formatter: formatter,
/**
...
//	},
	...
	_initTaskProcess: function (type) {
		...
		list.setBusy(true)
		this.JsonModel().loadData(this,"bpm/findTaskProcess?type="+type,
	...
	_initApplyHistory: function (type) {
		...
		list.setBusy(true)
		this.JsonModel().loadData(this,"bpm/findStarterProcess?type="+type,
			function (oData, model) {
				if (oData.statusCode == "000000") {
					$.each(oData.data, function(index, ele) {
						ele.startTime = this.convertDate(ele.startTime);
	...
	_initTaskHistory: function (type) {
		...
		list.setBusy(true)
		this.JsonModel().loadData(this,"bpm/findHistoryProcess?type="+type,
			function (oData, model) {
				if (oData.statusCode == "000000") {
					$.each(oData.data, function(index, ele) {
						ele.startTime = this.convertDate(ele.startTime);
	...
	_filter: function(listId,oEvent){
		...
		if (sQuery && sQuery.length > 0) {
			_oGlobalFilter = new Filter([
				new Filter("businessKey", FilterOperator.Contains, sQuery),
				new Filter("description", FilterOperator.Contains, sQuery),
				new Filter("startUserId", FilterOperator.Contains, sQuery),
	...
	onProcessPress: function (oEvent) {
		...
		context = oList.getBindingContext("process");
		//查看明细
		this._showInfo(context,true);
	},
	onHistoryPress: function (oEvent) {
		var oList = oEvent.getSource(),
		context = oList.getBindingContext("history");
	...
	onApplyPress: function (oEvent) {
		var oList = oEvent.getSource(),
		context = oList.getBindingContext("apply");
