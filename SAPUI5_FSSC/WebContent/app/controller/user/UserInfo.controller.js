sap.ui.define([ 
	...
	'harry/app/format/formatter'
				...
				formatter) {
	"use strict";
	return Controller.extend("harry.app.controller.user.UserInfo", {
		formatter: formatter,
/**
...
//	},
	...
	initRoles: function(oEvent){
		...
		this.JsonModel().loadData(this,"role/listByUser?userid="+userId,
	...
	initRecent: function(oEvent){
		var userId=this.getView().getModel("target").getData().data.userId;
		this.JsonModel().loadData(this,"behavior/recentActivities?id="+userId,
			function(oData, model) {
				if (oData.statusCode == "000000") {
					$.each(oData.data, function(index, ele) {
	...
	initAll: function(oEvent){
		var userId=this.getView().getModel("target").getData().data.userId;
		this.JsonModel().loadData(this,"behavior/allActivities?id="+userId,
			function(oData, model) {
				if (oData.statusCode == "000000") {
					$.each(oData.data, function(index, ele) {
						ele.date = this.convertDate(ele.date);
